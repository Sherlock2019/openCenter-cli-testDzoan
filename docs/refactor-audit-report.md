...
| `internal/cluster/sync/openstack/` | OpenStack synchronization workflow |
...
| Atomic writes in `internal/util/files`, `internal/util/fs`, `internal/gitops/atomic.go`, OpenStack sync, and `cmd/secrets_sops_helpers.go` | All aim to avoid partial replacement, but contracts differ: retry policy, temp naming, permission preservation, `fsync`, staged workspace semantics, rollback, structured errors, and external SOPS execution. Much of the duplication is intentional. | Each major domain has tests, but cross-platform filesystem semantics are not expressed as one contract. Risk: high. | Do not merge wholesale. At most extract a narrowly specified `internal/atomicfile` replacement primitive after tests define mode preservation, durability, cleanup, and Windows behavior. Keep GitOps transactions separate. **Leave alone unless Prompt 3 explicitly characterizes the contract.** |
